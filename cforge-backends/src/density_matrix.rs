    ...
    fn run(
        ...
        for (op_idx, op) in circuit.operations.iter().enumerate() {
            ...
            if let Some(ref cfg) = self.config {
                ...
                let noise_ch = if is_two_qubit {
                    cfg.two_qubit.as_ref()
                } else {
                    cfg.single_qubit.as_ref()
                };
        ...
        let statevector: Vec<Complex64> = probs
            .iter()
...
fn apply_ccx_dm(rho: &mut [Complex64], n: usize, c0: usize, c1: usize, tgt: usize) {
    ...
    let bt = 1 << tgt;
    for i0 in 0..big_n {
        if (i0 & b0 == 0) || (i0 & b1 == 0) || (i0 & bt != 0) {
            continue;
        }
        let i1 = i0 | bt;
        for j0 in 0..big_n {
            if (j0 & b0 == 0) || (j0 & b1 == 0) || (j0 & bt != 0) {
                continue;
            }
            let j1 = j0 | bt;
...
fn apply_cswap_dm(rho: &mut [Complex64], n: usize, ctrl: usize, a: usize, b: usize) {
    ...
    for i0 in 0..big_n {
        if (i0 & cb == 0) || (i0 & ba != 0) || (i0 & bb == 0) {
            continue;
        }
        let i1 = (i0 | ba) & !bb;
        for j0 in 0..big_n {
            if (j0 & cb == 0) || (j0 & ba != 0) || (j0 & bb == 0) {
                continue;
            }
            let j1 = (j0 | ba) & !bb;
...
fn gate_t() -> U2 {
    let e = Complex64::from_polar(1., PI / 4.);
    [[c(1., 0.), c(0., 0.)], [c(0., 0.), e]]
}
fn gate_tdg() -> U2 {
    let e = Complex64::from_polar(1., -PI / 4.);
...
fn gate_rz(t: f64) -> U2 {
    [
        [Complex64::from_polar(1., -t / 2.), c(0., 0.)],
        [c(0., 0.), Complex64::from_polar(1., t / 2.)],
    ]
}
fn gate_phase(t: f64) -> U2 {
    [
        [c(1., 0.), c(0., 0.)],
        [c(0., 0.), Complex64::from_polar(1., t)],
    ]
}
...
mod tests {
    ...
    fn depolarizing_noise_degrades_fidelity() {
        ...
        let noisy = DensityMatrixBackend {
            config: Some(NoisyConfig::depolarizing(0.3, 0.3)),
        };
        ...
        assert!(
            max_p < 0.5,
            "noisy Bell state max probability should be < 0.5, got {max_p}"
        );
    }
    ...
    fn amplitude_damping_decays_excited_state() {
        ...
        let noisy = DensityMatrixBackend {
            config: Some(NoisyConfig::amplitude_damping(0.9)),
        };
