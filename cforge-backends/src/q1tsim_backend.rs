...
use q1tsim::gates::{
    CCX, CH, CRX, CRY, CRZ, CX, CY, CZ, CU1, CU3, CV, Gate, H, I,
    RX, RY, RZ, S, Sdg, Swap, T, Tdg, U1, U3, V, Vdg, X, Y, Z,
};
...
fn m1(gate: impl Gate) -> Result<U2, String> {
    let m = gate.matrix();
    Ok([[q2c(&m[[0,0]]), q2c(&m[[0,1]])], [q2c(&m[[1,0]]), q2c(&m[[1,1]])]])
}

fn m2(gate: impl Gate) -> Result<U4, String> {
    let m = gate.matrix();
    let mut out = [[Complex64::new(0.0,0.0); 4]; 4];
    for i in 0..4 { for j in 0..4 { out[i][j] = q2c(&m[[i,j]]); } }
    Ok(out)
}
...
fn m3(gate: impl Gate) -> Result<U8, String> {
    ...
    for i in 0..8 { for j in 0..8 { out[i][j] = q2c(&m[[i,j]]); } }
    Ok(out)
}
...
fn apply3(sv: &mut [Complex64], q0: usize, q1: usize, q2: usize, u: U8) {
    let bit0=1<<q0; let bit1=1<<q1; let bit2=1<<q2;
    for i in 0..sv.len() {
        if (i&bit0!=0)||(i&bit1!=0)||(i&bit2!=0) { continue; }
        let idx=[i,i|bit2,i|bit1,i|bit1|bit2,i|bit0,i|bit0|bit2,i|bit0|bit1,i|bit0|bit1|bit2];
        let old:[Complex64;8] = std::array::from_fn(|k| sv[idx[k]]);
        for row in 0..8 { sv[idx[row]]=(0..8).map(|col| u[row][col]*old[col]).sum(); }
    }
}
...
mod tests {
    ...
    fn rz_on_superposition_q1tsim_agrees_with_native() {
        ...
        c.push(Operation::new(GateKind::H,  vec![0], vec![]));
        c.push(Operation::new(GateKind::Rz, vec![0], vec![std::f64::consts::FRAC_PI_4]));
        let (sv1, sv2) = run_both(&c);
